...
class Coffee extends EventEmitter {
  ...
  expect(type, expected) {
    this.addAssertion({ type, expected });
    return this;
  }

  notExpect(type, expected) {
    this.addAssertion({ type, expected, isOpposite: true });
    return this;
  }

  addAssertion({ type, expected, isOpposite }) {
    if (acceptType.indexOf(type) === -1) return;

    const rule = new Rule({ expected, type, isOpposite });
    if (this.complete) {
      rule.assert(this);
    } else {
  ...
  checkAssertion() {
    for (const rule of this.waitAssert.stdout) {
      rule.assert(this);
    }

    for (const rule of this.waitAssert.stderr) {
      rule.assert(this);
    }

    for (const rule of this.waitAssert.code) {
      rule.assert(this);
    }

    if (this.error) {
      for (const rule of this.waitAssert.error) {
        rule.assert(this);
      }
    }
