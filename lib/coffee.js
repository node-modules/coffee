...
class Coffee extends EventEmitter {
  ...
  _hookEvent() {
    ...
    this.on('message', msg => {
      debug('message `%s`', msg);
      if (this._waitMessage.includes(msg)) {
        this.emit('close', 0);
      }
    });
  ...
  /**
   * when child_process message emit, will resolve `end()`
   *
   * @param {String} msg - message
   * @return {Coffee} return self for chain
   */
  waitForMessage(msg) {
    this._waitMessage.push(msg);
    return this;
  }

  /**
   * kill proc when timeout and trigger close
   *
  ...
  _run() {
    ...
    cmd.on('message', this.emit.bind(this, 'message'));
